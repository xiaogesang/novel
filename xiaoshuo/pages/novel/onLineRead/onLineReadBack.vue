		...
		<uni-drawer maskClick ref="showRight" mode="right" :mask-click="false">
		    <scroll-view  style="position: relative;height: 100%;background-color: #fffdf3;padding-left: 20rpx;" scroll-y="true">
		       <view class="prebottom" style="display: flex;">
		       	<view @click="pres" class="pre">上一页</view>
		       	<view @click="nexts" class="pre">下一页</view>
		       </view>
				<view @click="choose(item)" class="charts-items" v-for="item in allcharList" :key="item.index"> {{item.title}}</view>
...
<script>
	import {getBackContent,getBackCharts} from '../../../api/index.js'
	...
	export default {
		...
		methods: {
			...
			getBackContent(){
				getBackContent({id:this.id,key:this.key}).then(res=>{
					this.next = res.data.next.id
					this.pre = res.data.prev.id
					this.deatail = res.data.content
					this.name = res.data.title
					uni.hideLoading()
				})
			},
			nextChart(){
				getBackContent({id:this.id,key:this.next}).then(res=>{
					this.next = res.data.next.id
					this.pre = res.data.prev.id
					this.deatail = res.data.content
					this.name = res.data.title
			...
			preChart(){
				getBackContent({id:this.id,key:this.pre}).then(res=>{
					this.next = res.data.next.id
					this.pre = rsa.data.prev.id
					this.deatail = res.data.content
					this.name = res.data.title
					uni.hideLoading()
				})
