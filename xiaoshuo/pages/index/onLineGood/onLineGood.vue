<template>
	<view>
		...
		<!-- <image :src="newimg(item.cover)" v-for="item in list" :key="item.index">
...
<script>
	import {GoodRank} from '../../../api/index.js'
	export default {
		data() {
			return {
				time:'',
		...
		methods: {
			...
			gettime(){
				let a = new Date()
				this.time = a.getFullYear()+'-'+(a.getMonth()+1)+'-'+a.getDate()
				console.log(this.time);
				GoodRank({page:'1',size:'10'}).then(res=>{
					console.log(res);
